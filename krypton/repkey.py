...
INT_CHAR_FREQS = {
    'e': 27, 't': 26, 'a': 25, 'o': 24, 'i': 23, 'n': 22,
    's': 21, 'r': 20, 'h': 19, 'l': 18, 'd': 17, 'c': 16,
    'u': 15, 'm': 14, 'f': 13, 'p': 12, 'g': 11, 'w': 10,
    'y': 9, 'b': 8, 'v': 7, 'k': 6, 'x': 5, ' ': 4,
    'j': 3, 'q': 2, 'z': 1
}
...
def break_single_char_key(cipher_text, key_chars=ascii_uppercase,
...
def break_rep_key(cipher_text, key_len, encrypt_func=vigenere):
    ...
    if key_len > 1:
        chunks = [
            cipher_text[i:i+key_len]
            for i in range(0, len(cipher_text), key_len)
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-k', '--key',
        help='Provide a specific key to use for decryption.'
    )
    parser.add_argument(
        '-l', '--keylength',
        type=int,
        help='Specify a single known keylength to break.'
    )
    parser.add_argument(
        '-a', '--auto',
        type=int,
    ...
    cipher_text = ''.join([
        c for c in sys.stdin.read().strip()
        if c != ' '
    ])
    ...
    if args.key:
        ...
        candidate = determine_key_length(
            cipher_text,
            max_key_length,
            encrypt_func
        )
